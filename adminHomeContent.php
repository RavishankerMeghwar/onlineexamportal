...
//settings
$admin_users = mysqli_query($conn, 'SELECT * FROM admin_users');
$admin_data = mysqli_fetch_all($admin_users, MYSQLI_ASSOC);
if (isset($_POST['settings'])) {
	$admin_name = $_POST['username'];
	$admin_pass = $_POST['password'];
	$admin_role = $_POST['role'];
	$settings = "INSERT INTO admin_users(username, password, role) VALUES('$admin_name','$admin_pass','$admin_role')";
	mysqli_query($conn, $settings);
	header("Location: adminHome.php");
}
?>
<!---Subject--->
		...
		<tbody>
			...
			foreach ($subjects as $subject) : ?>
				<tr>
					...
					<td><a href='deletecourse.php?subject_id= <?php echo $subject['subject_id']; ?> ' class="btn btn-danger">Delete Subject</a></td>
				</tr>
			<?php $i++;
			endforeach; ?>
		</tbody>
		...
		<tbody>
			<?php foreach ($tests as $test) : ?>
				<tr>
					...
					<td><a href='deletetest.php?test_id=<?php echo $test['test_id']; ?> ' class="btn btn-primary">Delete Subject</a></td>
				</tr>
			<?php endforeach; ?>
		</tbody>
	</table>
...
<div id="settings" class="tab-pane fade">
	...
	<form class="form-inline">
		...
		<div class="form-group"><select class="form-control" name="role">
				<option>Admin</option>
				<option>Instructor</option>
			</select></div>
		<input class="btn btn-success" type="submit" name="settings" value="Save">
	</form>
